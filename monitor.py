    ...
    for device in devices.values():
        yaml_devices.append(
            {'name': device.name.encode('ascii', 'ignore').decode('ascii'),
             'ip': device.host[0], 'port': device.host[1], 'mac': device.status['macaddress']}
        )
...
def read_config(config_file_path):
    ...
    config["mqtt_client_id"] = config_file["mqtt"]["client_id"] if config_file[
        "mqtt"]["client_id"] else 'broadlink_to_mqtt-'+str(time.time())
...
def start():
    ...
    # HomeAssistant stuff
    parser.add_argument("-Hd", "--dumphaconfig",
                        help="Dump the devices as a HA manual config entry", action="store_true", default=False)
    ...
    # Config helpers
    parser.add_argument("-S", "--discoverdump",
                        help="Discover devices and dump config", action="store_true", default=False)
    ...
    # Generic
    parser.add_argument("-s", "--discover", help="Discover devices",
                        action="store_true", default=False)
    parser.add_argument("-d", "--debug", help="set logging level to debug",
                        action="store_true", default=False)
    parser.add_argument("-v", "--version",
                        help="Print Verions", action="store_true")
    parser.add_argument(
        "-dir", "--data_dir", help="Data Folder -- Default to folder script is located", default=False)
    parser.add_argument(
        "-c", "--config", help="Config file path -- Default to folder script is located + 'config.yml'", default=False)
    parser.add_argument(
        "-l", "--logfile", help="Logfile path -- Default to logs folder script is located", default=False)
    parser.add_argument("-T", "--test", help="send test set temperature packet, for testing only",
                        action="store_true", default=False)
    ...
    if args.logfile:
        ...
        log_file_path = os.path.dirname(
            os.path.realpath(__file__))+'/log/out.log'

    # Log level: -d/--debug forces DEBUG; otherwise honor the LOGLEVEL env var
    # (default INFO) so verbosity can be tuned without editing the Dockerfile.
    ...
    logLevel = {0: 'NOTSET', 10: 'DEBUG',
                20: 'INFO', 30: 'WARNING', 40: 'ERROR'}
    logger.debug('Loglevel set to ' +
    ...
    # Print verions
    if args.version:
        print("Monitor Version: %s, Class version:%s" %
              (softwareversion, ac_db_version.version))
        sys.exit()
