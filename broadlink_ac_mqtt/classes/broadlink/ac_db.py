...
class ac_db(device):
    ...
    class STATIC:
        ...
        class FAN:
            LOW = 0b00000011   # 3  low (app 50%)
            MEDIUM = 0b00000010   # 2  medium (IR-remote only)
            MEDIUM_HIGH = 0b00000001   # 1  medium-high (app 75%)
            HIGH = 0b00000101   # 5  high (app 100%, canonical)
            # 6  high variant (IR-remote only, between 75% and 100%); shown as High
            HIGH_ALT = 0b00000110
            AUTO = 0b00000111   # 7  auto (cooling mode)
            NONE = 0b00000000
    ...
    def __init__(self, host, mac, name=None, cloud=None, debug=False, update_interval=0, devtype=None, bind_to_ip=None):
        ...
        device.__init__(self, host, mac, name=name, cloud=cloud,
                        devtype=devtype, update_interval=update_interval)
    ...
    def set_fixation_v(self, fixation_text):
        ...
        fixation = self.STATIC.FIXATION.VERTICAL.__dict__.get(
            fixation_text.upper())
        if fixation != None:
    ...
    def set_fixation_h(self, fixation_text):
        ...
        fixation = self.STATIC.FIXATION.HORIZONTAL.__dict__.get(
            fixation_text.upper())
        if fixation != None:
    ...
    def get_ac_info(self):
        ...
        if err == 0:
            ...
            self.logger.debug("Acinfo Raw Response: " +
                              ' '.join(format(x, '08b') for x in response_payload))
            self.logger.debug(
                "Acinfo Raw Hex: " + ' '.join(format(x, '02x') for x in response_payload))

            # Drop leading stuff as dont need
            response_payload = response_payload[2:]
            self.logger.debug("AcInfo: " + ' '.join(format(x, '08b')
                              for x in response_payload[9:]))
            ...
            self.logger.debug("Ambient Temp Decimal: %s" % float(
                response_payload[31] & 0b00011111))  # @Anonym-tsk
            ...
            self.logger.debug("Failed Raw Response: " +
    ...
    def get_ac_states(self, force_update=False):
        ...
        if err == 0:
            ...
            self.logger.debug("Raw AC Status: " + ' '.join(format(x, '08b')
                              for x in response_payload[9:]))
            ...
            self.status['temp'] = 8 + (response_payload[10] >> 3) + \
    ...
    def make_nice_status(self, status):
        ...
        status_nice['power'] = self.get_key(
            self.STATIC.ONOFF.__dict__, status['power'])
        status_nice['fixation_v'] = self.get_key(
            self.STATIC.FIXATION.VERTICAL.__dict__, status['fixation_v'])
        status_nice['mode'] = self.get_key(
            self.STATIC.MODE.__dict__, status['mode'])
        status_nice['sleep'] = self.get_key(
            self.STATIC.ONOFF.__dict__, status['sleep'])
        ...
        status_nice['ifeel'] = self.get_key(
            self.STATIC.ONOFF.__dict__, status['ifeel'])
        status_nice['mute'] = self.get_key(
            self.STATIC.ONOFF.__dict__, status['mute'])
        status_nice['turbo'] = self.get_key(
            self.STATIC.ONOFF.__dict__, status['turbo'])
        status_nice['clean'] = self.get_key(
            self.STATIC.ONOFF.__dict__, status['clean'])
        ...
        # HIGH_ALT (raw=1) is the same user-facing fan speed as HIGH (raw=5);
        # normalise so both display as HIGH / High.
        if status_nice['fanspeed'] == "HIGH_ALT":
            status_nice['fanspeed'] = "HIGH"
    ...
    def get_key(self, list, search_value):
        ...
        for key, value in list.items():
            if value == search_value:
                return key
        # Not found so return value as a string; callers (e.g. .title()) expect
        # a string, and an unmapped raw int has no such method.
        return str(search_value)
    ...
    def set_ac_status(self):
        ...
        self.logger.debug("Packet:" + ''.join(format(x, '02x')
                          for x in request_payload))

        response = self.send_packet(0x6a, request_payload)
        self.logger.debug("Resposnse:" + ''.join(format(x, '02x')
                          for x in response))
        ...
        if err == 0:
            ...
            self.logger.debug("Payload: Nice:" + ''.join(x.encode('hex')
