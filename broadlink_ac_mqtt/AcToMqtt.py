    ...
    def start (self,config, devices = None):
        ...
        try:
            ...
            if all(key in self.last_update for key in devices):
                next_due = min(self.last_update[key] + interval for key in devices)
                if next_due > now:
    ...
    def make_devices_array_from_devices(self,devices):
        ...
        for device in devices.values():
            ...
            device_array = { 
                ...
                ,"fan_modes": ["Auto","Low","Medium","Medium_High", "High","Turbo","Mute"]
    ...
    def connect_mqtt(self):
        ...
        if self._mqtt is not None:
            if not self._mqtt.is_connected():
                logger.warning("MQTT not currently connected; relying on paho auto-reconnect for %s" % self.config["mqtt_host"])
            return
